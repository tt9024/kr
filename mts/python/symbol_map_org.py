...
TradableKeys = ["symbol", "exch_symbol", "venue", "tick_size", "point_value", "px_multiplier", "type", "mts_contract", "contract_month", "mts_symbol", "N", "expiration_days", "tt_security_id", "tt_venue", "currency", "expiration_date","bbg_id","bbg_px_multiplier", "tickdata_id", "tickdata_px_multiplier", "tickdata_timezone", "lotspermin", "start_time", "end_time"]
...
class SymbolMap :
    def __init__(self, xml_path = '/home/mts/run/config/symbol', max_N = 2, assets_xml = "assets.xml") :
    ...
    def get_tinfo(self, mts_or_tradable, yyyymmdd = None) :
        if yyyymmdd is None :
            yyyymmdd = datetime.datetime.now().strftime('%Y%m%d')
        try :
            # try it as mts symbol, 
            smap = self.get_tradable_map(yyyymmdd, mts_key = True, mts_symbols = None)
            return smap[mts_or_tradable]
        except :
            try :
                smap = self.get_tradable_map(yyyymmdd, mts_key = False, mts_symbols = None)
                return smap[mts_or_tradable]
            except :
                raise RuntimeError("failed to get tinfo " + mts_or_tradable)
            
    def list_symbol(self, today_yyyymmdd = None, mts_symbol_list=None) :
        """
        ...
        smap = self.get_tradable_map(today_yyyymmdd, mts_key = True)
    ...
    def get_symbol_contract_from_tradable(self, tradable, trade_day = None) :
        """
        return mts_symbol, symbol and  contract_month
        """
        tinfo = self.get_tinfo(tradable, trade_day)
