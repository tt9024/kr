...
public:
	...
	std::string ToString() const{
		...
		oss << "Contract{" << PrintVar(conId) << PrintVar(symbol)
				<< PrintVar(secType) << PrintVar(expiry)
				<< PrintVar(exchange) << PrintVar(primaryExchange)
				<< PrintVar(currency) << PrintVar(localSymbol)
				<< PrintVar(tradingClass)<< PrintVar(secIdType) <<
				PrintVar(secId) << "}";
		return oss.str();
	}
};
...
struct ContractDetails
{
	...
	std::string ToString() const {
		std::ostringstream oss;
		oss << "ContractDetail{" << summary.ToString() << PrintVar(marketName) << PrintVar(minTick)
			<< PrintVar(orderTypes) << PrintVar(validExchanges)
			<< PrintVar(priceMagnifier) << PrintVar(underConId) << PrintVar(longName)
			<< PrintVar(industry) << PrintVar(category) << PrintVar(notes) << "}";
		return oss.str();
	}
};
