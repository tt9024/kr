...
def read_ar1_config(fname_cfg, symbol) :
    ...
    pkeys_arr = [('fscale',float),('vscale',float),('ixf',int),('ixf_st',int),('pick',int)]
    for (key,tp) in pkeys :
...
def get_pos_param(f, p0, exec_param_daily, use_k=None, logfunc=NullLogger) :
...
def get_p0(k,f,exec_param,prev_pos,logfunc=NullLogger) :
...
class AR1_Model :
    ...
    def on_snap(self, k, a, md_dict):
        ...
        p0=pp0[-1]
        self.fm.prev_pos=p0
        self.logger.logInfo('on snap (%i,%f): pos:(%f)\nf:(%s)\nv:(%s)'%(k,a,p0,str(f),str(v)))
