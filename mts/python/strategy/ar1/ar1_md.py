...
def md_dict_from_mts_col(symbol, start_day, end_day, barsec, cols = ['open', 'close', 'vol','vbs','lpx','utc', 'absz','aasz','aspd','bdif','adif'],  use_live_repo=True):
    if use_live_repo:
        repo = mts_repo.MTS_REPO_Live()
    else:
        repo = mts_repo.MTS_REPO_TickData()
    bars, roll_adj_dict = repo.get_bars(symbol, start_day, end_day, barsec=barsec, cols = cols, is_mts_symbol=True, ignore_prev=False, remove_zero=False, get_roll_adj=True)
...
def md_dict_from_mts(symbol, start_day, end_day, barsec, use_live_repo=True, roll_adj_lpx=False):
    ...
    return:
        bar: shape(ndays,n,5)) for [utc, lr, vol, vbs, lpx]
        Note both the lr and lpx are roll adjusted
    """
    cols = ['open', 'close', 'vol','vbs','lpx','utc']
    bars, roll_adj_dict = md_dict_from_mts_col(symbol, start_day, end_day, barsec, cols=cols, use_live_repo=use_live_repo)
