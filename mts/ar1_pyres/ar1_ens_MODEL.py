...
Default_REF_Config = '/home/mts/upload/ZFU_STRATS/INTRADAY_MTS_AR1_WTI_US/LiveHO-INTRADAY_MTS_AR1_WTI_US.cfg'
Default_Logger_File = '/tmp/ar1'
Default_REF_Weight = 1
Default_ENS_Weight = 0
MKT = 'WTI_N1'

class AR1_WTI_Ens:
    def __init__(self, mkt, logger_file, ens_cfg_fname, ens_weight=1, ref_cfg_fname=None, ref_weight=0):
    ...
    def start(self):
        ...
        # intialize the md
        self.md.sod()
        while cur_utc < t1 + self.barsec and k < self.n:
            try :
                ...
                if trigger_utc[tr_ix] <= cur_utc :
                    ...
                    p0 = self._ens_on_snap(k, a, md_dict)
    ...
    def _ens_on_bar(self, k, md_dict):
        ...
        p0 = ar1_sim_ens.agg_pos_day(self.mp_dict, pos_dict, karr, log_func=self.logger.logInfo)[0]*self.ens_weight
        self.logger.logInfo('ens_on_bar (k=%d): pos_dict(%s), ens_weight(%f), p0(%f)'%(k, str(pos_dict), self.ens_weight, p0))
        if self.mdl_ref is not None:
            ens_maxpos = self.mp_dict['ens_maxpos']
            p0_ref = self.mdl_ref.on_bar(k, md_dict)
            p0 = np.clip(p0+self.ref_weight*p0_ref, -ens_maxpos, ens_maxpos)
            self.logger.logInfo('ref_on_bar(k=%d): p0_ref(%f), ref_weight(%f), ens_maxpos(%d), p0(%f)'%(k,p0_ref, self.ref_weight, ens_maxpos, p0))
        return p0
    ...
    def _ens_on_snap(self, k, a, md_dict):
        ...
        for mdl_id in self.mdl_dict.keys():
            ...
            pos_dict[mdl_id] = np.array([p0])
        p0 = ar1_sim_ens.agg_pos_day(self.mp_dict, pos_dict, karr, log_func=self.logger.logInfo)[0]*self.ens_weight
        self.logger.logInfo('ens_on_snap (k=%d,a=%.2f): pos_dict(%s), ens_weight(%f), p0(%f)'%(k,a,str(pos_dict), self.ens_weight, p0))
        if self.mdl_ref is not None:
            ens_maxpos = self.mp_dict['ens_maxpos']
            p0_ref = self.mdl_ref.on_snap(k, a, md_dict)
            p0 = np.clip(p0+self.ref_weight*p0_ref, -ens_maxpos, ens_maxpos)
            self.logger.logInfo('ref_on_snap(k=%d,a=%.2f): p0_ref(%f), ref_weight(%f), ens_maxpos(%d), p0(%f)'%(k,a,p0_ref, self.ref_weight, ens_maxpos, p0))
        return p0
    ...
    def _set_pos(self, qty, symbol = 'WTI_N1'):
        ...
        run_str_tsc = ['/home/mts/run/bin/flr',  'X',  'TSC-7000-'+str(self.mp_dict['strat_code']) + ', ' + symbol + ', ' + str(np.round(qty*self.sw_tsc)) + ', Y10m']
        run_str_tsd = ['/home/mts/run/bin/flr',  'X',  'TSD-7000-'+str(self.mp_dict['strat_code']) + ', ' + symbol + ', ' + str(np.round(qty*self.sw_tsd)) + ', Y10m']
        try :
...
if __name__ == "__main__":
    while True:
        ...
        if weekday < 4 or (weekday == 4 and dtnow.hour < 17) or (weekday==6 and dtnow.hour>= 17):
            break
        time.sleep(5)
    mdl = AR1_WTI_Ens(MKT, Default_Logger_File, Default_ENS_Config, Default_ENS_Weight, Default_REF_Config, Default_REF_Weight)
    mdl.start()
