...
class VBS :
    ...
    def ind2(self, decay_Li = 0.8, rollwnd = 50, ewa = None, bucket_hour = 1.0, vbs = None, save_state=False) :
        """
    ...
    def ind000(self) :
        """
        just the vbs as the test_default
        """
        return self.vbs

    def ind_test(self, initdays=50, ewa = 0.5) :
        """
        buy/sell break out w.r.t. normal range for the bar time
        if ewa is not None, then the result is exponentially moving avg'ed:
        S(t+1) = (1-ewa) * S(t) + ewa * X(t+1), where X(t+1) is the sample at t+1
        ...
        vbs2=np.cumsum(np.hstack((np.vstack((np.zeros(n),self.vbs[:-1,:])),self.vbs)),axis=1)
        #vbs2=np.cumsum(np.tile(self.vbs,2),axis=1) ## look ahead

        ix1=np.arange(n).astype(int)-self.Li+n-1
        ix2=np.arange(n).astype(int)+n
        x=vbs2[:,ix2]-vbs2[:,ix1]

        # add init days for running initialization
        y0 = np.tile((np.arange(ndays+initdays)+1).reshape((ndays+initdays,1)),(1,n))
        x0 = np.vstack((x[:initdays,:],x))
        x0m = np.cumsum(x0,axis=0)/y0
        x0std = np.sqrt(np.cumsum((x0m-x0)**2,axis=0)/y0)
        x = (x0[initdays:,:]-x0m[initdays-1:-1,:])/x0std[initdays-1:-1,:]
