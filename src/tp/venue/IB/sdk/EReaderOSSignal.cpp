...
int EReaderOSSignal::waitForSignal() {
	int errcode = 0;
#if defined(IB_POSIX)
    pthread_mutex_lock(&m_mutex); 
    if (!open) {
        if ( m_waitTimeout == INFINITE ) {
        	errcode=pthread_cond_wait(&m_evMsgs, &m_mutex);
        }
        else {
            struct timespec ts;
#if defined(IBAPI_MONOTONIC_TIME)
...
// on Mac OS X, clock_gettime is not available, stick to gettimeofday for the moment
            ...
            errcode=pthread_cond_timedwait(&m_evMsgs, &m_mutex, &ts);
        }
    }
    open = false;
    pthread_mutex_unlock(&m_mutex);
#elif defined(IB_WIN32)
	errcode=WaitForSingleObject(m_evMsgs, m_waitTimeout);
#else
#   error "Not implemented on this platform"
#endif
	return errcode;
